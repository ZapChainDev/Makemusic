...
async function generateImagePrompt(title) {
  console.log("  🎨 Generating image scene from blog title...");
  const sceneResponse = await getOpenAI().chat.completions.create({
    model: "gpt-4o-mini",
    messages: [
      {
        role: "user",
        content: `You are a creative director for a custom personalized song website. Given a blog post title, describe ONE specific, emotionally vivid scene for a photo that directly matches the post's topic.

Blog post title: "${title}"

Rules:
- Feature real people in an authentic, emotional human moment that fits the post's specific occasion or theme
- Be specific and cinematic: name the action, the emotion, the setting detail
- Good examples: "a man getting down on one knee at sunset on the beach, his girlfriend covering her mouth in joyful shock", "a family gathered around a birthday cake, grandma blowing out candles while grandchildren cheer", "two old friends embracing tightly at an airport arrivals gate, both in happy tears"
- NO instruments, NO musicians, NO guitars, NO recording studios, NO microphones
- 1–2 sentences only, vivid and concrete
- Output ONLY the scene description, nothing else`,
  ...
  const scene = sceneResponse.choices?.[0]?.message?.content?.trim();
  if (!scene) throw new Error("No image scene generated by OpenAI");
  console.log("  🖼️  Scene:", scene);
  const styles = [
    "cinematic lifestyle photography",
    "warm golden-hour editorial",
    "soft natural light portrait photography",
    "authentic documentary-style photography",
    "vibrant feel-good lifestyle photography",
    "moody intimate close-up photography",
  ];
  const style = styles[Math.floor(Math.random() * styles.length)];
  return `${style}. ${scene}. Real people, genuine emotion, authentic human connection. No instruments, no musicians, no stock photo feel, no text overlay, no logos.`;
}
